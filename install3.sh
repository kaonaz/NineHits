...
#url="http://192.168.0.102/9hits/9hitsv3-linux64.tar.bz2"
url="http://rs.9hits.com/app/9hitsv3-linux64.tar.bz2"
...
CACHE_DEL=""
SSH_CONNECTOR=""
INSTALL_DIR=/usr/local/ninehits/
...
while [[ ! \$(pidof 9hits) ]]; do
	...
	Xvfb :1 &
	export DISPLAY=:1 && $INSTALL_DIR/9hitsv3-linux64/9hits --auto-start --single-process --hide-app --disable-crash-handler=true --disable-logging > /dev/null
	exit
done
EOL
...
function install_apt() {
	apt-get update
	apt-get upgrade -y
	apt-get install -y bzip2 libcanberra-gtk-module libxss1 xvfb htop sed tar libxtst6 libnss3 wget psmisc bc libgtk-3-0 libgbm-dev libatspi2.0-0 libatomic1 && install_9hits
}
